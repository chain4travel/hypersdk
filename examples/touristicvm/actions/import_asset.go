...
func (i *ImportAsset) executeMint(
	...
	exists, symbol, decimals, metadata, supply, maxSupply, _, warp, err := storage.GetAsset(ctx, mu, asset)
	if err != nil {
	...
	if err := storage.SetAsset(ctx, mu, asset, symbol, decimals, metadata, newSupply, maxSupply, codec.EmptyAddress, true); err != nil {
		return utils.ErrBytes(err)
	}
...
func (i *ImportAsset) executeReturn(
	...
	exists, symbol, decimals, _, _, _, _, warp, err := storage.GetAsset(ctx, mu, i.warpTransfer.Asset)
	if err != nil {
		return utils.ErrBytes(err)
	}
	if !exists {
