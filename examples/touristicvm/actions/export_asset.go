...
func (e *ExportAsset) executeReturn(
	...
	exists, symbol, decimals, metadata, supply, maxSupply, _, isWarp, err := storage.GetAsset(ctx, mu, e.Asset)
	if err != nil {
	...
	if newSupply > 0 {
		if err := storage.SetAsset(ctx, mu, e.Asset, symbol, decimals, metadata, newSupply, maxSupply, codec.EmptyAddress, true); err != nil {
			return false, ExportAssetComputeUnits, utils.ErrBytes(err), nil, nil
		}
	} else {
...
func (e *ExportAsset) executeLoan(
	...
	exists, symbol, decimals, _, _, _, _, isWarp, err := storage.GetAsset(ctx, mu, e.Asset)
	if err != nil {
		return false, ExportAssetComputeUnits, utils.ErrBytes(err), nil, nil
	}
	if !exists {
