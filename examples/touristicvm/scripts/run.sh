...
ADDRESS=${ADDRESS:-touristic1qrzvk4zlwj9zsacqgtufx7zvapd3quufqpxk5rsdd4633m4wz2fdju6xf8r}
if [[ ${MODE} != "run" ]]; then
  LOGLEVEL=debug
...
############################
echo "building touristicvm"

# delete previous (if exists)
rm -f ${TMPDIR}/avalanchego-${VERSION}/plugins/tHBmZEZ7iLgMugQKNvCufu5G6Qh8STKgdt6aRJUAVPc72gPYc

# rebuild with latest code
go build \
-o ${TMPDIR}/avalanchego-${VERSION}/plugins/tHBmZEZ7iLgMugQKNvCufu5G6Qh8STKgdt6aRJUAVPc72gPYc \
./cmd/touristicvm

echo "building touristic-cli"
go build -v -o ${TMPDIR}/touristic-cli ./cmd/touristic-cli
...
if [[ -z "${GENESIS_PATH}" ]]; then
  ...
  rm -f ${TMPDIR}/touristicvm.genesis
  ${TMPDIR}/touristic-cli genesis generate ${TMPDIR}/allocations.json \
  --window-target-units ${WINDOW_TARGET_UNITS} \
  --max-block-units ${MAX_BLOCK_UNITS} \
  --min-block-gap ${MIN_BLOCK_GAP} \
  --genesis-file ${TMPDIR}/touristicvm.genesis
else
  echo "copying custom genesis file"
  rm -f ${TMPDIR}/touristicvm.genesis
  cp ${GENESIS_PATH} ${TMPDIR}/touristicvm.genesis
fi
...
rm -f ${TMPDIR}/touristicvm.config
rm -rf ${TMPDIR}/touristicvm-e2e-profiles
cat <<EOF > ${TMPDIR}/touristicvm.config
{
  ...
  "logLevel": "${LOGLEVEL}",
  "continuousProfilerDir":"${TMPDIR}/touristicvm-e2e-profiles/*",
  "stateSyncServerDelay": ${STATESYNC_DELAY}
}
EOF
mkdir -p ${TMPDIR}/touristicvm-e2e-profiles
...
rm -f ${TMPDIR}/touristicvm.subnet
cat <<EOF > ${TMPDIR}/touristicvm.subnet
{
...
--vm-genesis-path=${TMPDIR}/touristicvm.genesis \
--vm-config-path=${TMPDIR}/touristicvm.config \
--subnet-config-path=${TMPDIR}/touristicvm.subnet \
